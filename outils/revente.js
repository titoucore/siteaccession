(function(){
  ...
  document.getElementById('btnEffacerIndices').addEventListener('click', function() {
    ...
    mettreAJourCompteur();
    mettreAJourDatesMaj();
    afficherStatut('irl-status', 'M\u00e9moire IRL effac\u00e9e.', '');
    afficherStatut('icc-status', 'M\u00e9moire ICC effac\u00e9e.', '');
  });
  ...
  function mettreAJourCompteur() {
    ...
    if (total === 0) {
      preview.innerHTML = '<em>Aucun indice en m\u00e9moire. Importez un fichier CSV depuis l\'INSEE.</em>';
      return;
    }
  ...
  function mettreAJourDatesMaj() {
    try {
      var irlDate = localStorage.getItem('o64-irl-updated-at');
      var iccDate = localStorage.getItem('o64-icc-updated-at');
      var irlName = localStorage.getItem('o64-irl-filename');
      var iccName = localStorage.getItem('o64-icc-filename');
      document.getElementById('irl-date').textContent = 'Derni\u00e8re mise \u00e0 jour\u00a0: '
        + (irlDate ? new Date(irlDate).toLocaleString('fr-FR') : 'aucune')
        + (irlName ? ' \u00b7 ' + irlName : '');
      document.getElementById('icc-date').textContent = 'Derni\u00e8re mise \u00e0 jour\u00a0: '
        + (iccDate ? new Date(iccDate).toLocaleString('fr-FR') : 'aucune')
        + (iccName ? ' \u00b7 ' + iccName : '');
    } catch(e) {}
  }
  ...
  // ─── ÉCOUTEURS ───
  ['dispositif','prix_initial','type_indice','indice_initial','indice_revente'].forEach(function(id) {
    var el = document.getElementById(id);
  ...
  (function() {
    ...
    function renderResults(d) {
      ...
      var h = '';
      h += row('Dispositif', d.dispositif ? (d.dispositif === 'brs' ? 'BRS — Bail Réel Solidaire' : 'PSLA') : null, !!d.dispositif);
      if (d.prix) {
...
(function initCourrier() {
  ...
  function syncCourrier() {
    ...
    var dispLabel = disp === 'brs' ? 'BRS — Bail Réel Solidaire' : disp === 'psla' ? 'PSLA' : '—';
    ...
    fillIfEmpty('cr_duree_clause',   acte.duree_clause || (disp === 'brs' ? '99 ans' : '10 ans'));
    ...
    if (clTA && !clTA.value) {
      ...
      if (clauseActeTxt) {
        ...
        clTA.value = "En cas de revente de tout ou partie du bien pendant cette période, le prix ne pourra être supérieur au prix d'achat figurant dans l'acte d'acquisition modifié en fonction de la variation de l'indice de référence des loyers (IRL) et augmenté, après acceptation par l'Office 64 de l'Habitat, du coût des travaux d'aménagement ou d'embellissement sur présentation des factures correspondantes, dans la limite de 10 % du prix d'accession.";
      } else {
        clTA.value = "En cas de revente de tout ou partie du bien pendant cette période, le prix ne pourra être supérieur au prix d'achat figurant dans l'acte d'acquisition modifié en fonction de la variation de l'indice du coût de la construction (ICC) et augmenté, après acceptation par l'Office 64 de l'Habitat, du coût des travaux d'aménagement ou d'embellissement sur présentation des factures correspondantes modifié en fonction de la variation de l'indice du coût de la construction.";
      }
    }
